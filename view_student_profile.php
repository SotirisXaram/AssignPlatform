...
$stmt = $conn->prepare("SELECT sa.*, ad.assignment_title, c.course_id, c.course_title 
...
$social_networks = ['linkedin', 'facebook', 'youtube', 'instagram', 'twitter'];
...
while ($assignment = $assignments_result->fetch_assoc()) {
    ...
    $description = $dom->createElement('description', $assignment['submission_description']);
    $assignment_elem->appendChild($description);
    
    $submission_file = $dom->createElement('submission_file', $assignment['submission_file']);
    $assignment_elem->appendChild($submission_file);
    
    $submission_date = $dom->createElement('submission_date', $assignment['submission_timestamp']);
    $assignment_elem->appendChild($submission_date);
    
    $submitted_assignments->appendChild($assignment_elem);
}

$xml_with_dtd = $dtd_declaration . "\n" . $dom->saveXML($dom->documentElement);

$validation_dom = new DOMDocument();

if (!$validation_dom->loadXML($xml_with_dtd)) {
	echo "<p>XSL Error Loading XML with DTD<p>";
	exit;  
}
else if (!$validation_dom->validate()) {
	echo "<p>XSL Error validate the xml<p>";
	exit;  
}
...
try {
    ...
    if (!$xsl->load('student_profile.xsl')) {
        throw new Exception("XSL error: Unable to load XSL file.");
    }
    ...
    $output = $proc->transformToXML($validation_dom);  
    if ($output === false) {
        throw new Exception("XSLT error: Transformation failed.");
    }
